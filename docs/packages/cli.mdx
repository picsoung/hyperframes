...
<Tabs>
  ...
  <Tab title="Build">
    ...
    | `--browser-timeout` | seconds | 60 | Puppeteer page-navigation timeout for the entry HTML. Increase when heavy compositions (many videos, fonts, or asset requests) cannot reach `domcontentloaded` within the default 60 s. Env fallback: `PRODUCER_PAGE_NAVIGATION_TIMEOUT_MS` (milliseconds). |
